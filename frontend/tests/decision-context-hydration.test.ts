...
test("页面通过 backend authority hydration，不从 URL query 推导 horizon/review_by", () => {
  ...
  const briefSource = readFileSync(new URL("../src/components/campaign/ResearchBrief.tsx", import.meta.url), "utf8");
  assert.match(briefSource, /data-horizon-source="CURRENT_THESIS"/);
  assert.match(briefSource, /来源：当前投资逻辑/);
