...
import { decisionActionLabel, decisionEvaluationLabel } from "@/lib/decisionActionView";
import {
...
export function DecisionProposalReview() {
  ...
  const handleGenerateAIDraft = async () => {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : "AI 草稿生成失败。");
    } finally {
      setAiBusy(false);
    }
  };
  ...
  const decisionReadbackSection = (<>
      {displayedDecision && (
        <section className="space-y-3 rounded-lg border border-success/40 bg-success/5 p-4" data-formal-decision-evaluation={evaluationOf(displayedDecision.formal_decision)} data-committed-decision-id={String(displayedDecision.committed.decision_id ?? "")} role="status">
          <h2 className="flex items-center gap-2 text-sm font-semibold"><CheckCircle2 className="h-4 w-4 text-success" />正式决策已保存并重新读取确认</h2>
          <p className="text-xs text-muted-foreground">当前决策状态：{decisionEvaluationLabel(evaluationOf(displayedDecision.formal_decision))} · 评估时间：<span className="font-mono">{displayedDecision.as_of}</span></p>
          ...
          <div className="grid gap-2 sm:grid-cols-3">
            <div className="rounded border border-border/50 bg-background/40 p-2 text-xs">正式投资逻辑：{decisionEvaluationLabel(evaluationOf(displayedDecision.formal_thesis))}</div>
            <div className="rounded border border-border/50 bg-background/40 p-2 text-xs">硬风险：{decisionEvaluationLabel(evaluationOf(displayedDecision.hard_risk))}</div>
            <div className="rounded border border-border/50 bg-background/40 p-2 text-xs">重大变化：{decisionEvaluationLabel(evaluationOf(displayedDecision.material_change))}</div>
          </div>
          <details className="text-[10px] text-muted-foreground">
            <summary className="cursor-pointer select-none hover:text-foreground">技术详情</summary>
            <p className="mt-1 font-mono">decision_id：{String(displayedDecision.committed.decision_id ?? "—")}</p>
            <p className="font-mono">evaluation：{evaluationOf(displayedDecision.formal_decision)}</p>
          </details>
          <p className="text-xs text-muted-foreground">{historicalId === null ? "决策待办会在下次刷新时读取这条记录；它是已确认的历史决策，不等于系统自动给出的当前建议。" : "此处只读展示指定历史决定；旧决定不会自动成为当前建议。"}</p>
          <div className="flex flex-wrap gap-x-4 gap-y-2">
            <Link to={historicalInboxHref} className="inline-flex text-xs text-primary hover:underline">打开决策待办 →</Link>
  ...
  if (historicalId !== null) return (
    <div className="space-y-4" data-testid="historical-decision-detail">
      <PageHeader title="历史正式决定" subtitle={`投资计划 ${campaignId} · 决定 ${historicalId}`} />
      <Link to={historicalInboxHref} className="text-primary underline">返回决策待办</Link>
  ...
  return (
    <div className="space-y-6" data-decision-proposal-page={campaignId}>
      ...
      {/* 来源：当前投资逻辑；摘要只读展示系统读取结果，不从 URL query 推导关注范围或复核时间 */}
      <ResearchBrief
        model={researchBrief}
