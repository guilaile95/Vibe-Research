...
export const airworthinessBlocks: ContentBlock[] = [
  {
    type: "paragraph",
    text: "适航认证是低空经济从概念走向商业化的核心门票。eVTOL 在中国需取得 CAAC 颁发的型号合格证（TC）、生产许可证（PC）和单机适航证（AC）。全球范围看，欧美中三地适航标准的互认程度和节奏差异将直接决定 eVTOL 制造商的出海能力和供应链布局。（分析推断）",
  },
  {
    type: "bullets",
    items: [
  ...
  },
  {
    type: "callout",
    tone: "warning",
    text: "适航节奏风险：eVTOL 适航进度是当前板块最大的不确定因素。乐观情景下海内外个别项目 2025–2026 年陆续取证、2027–2028 年有限商业运营；悲观情景下认证延期导致该时间表整体后移 2–3 年。两种情景差异来自适航当局人力资源配置、新技术标准的成熟度以及供应链对制造标准的适应速度。目前尚无公开资料确认任一情景的概率分布。",
  },
  {
    type: "paragraph",
    text: "商业运营条件：除适航取证外，eVTOL 商业运营还需满足（1）运营基地获得起降场使用许可；（2）运营商取得 CCAR-135/136 运行合格证；（3）空管部门完成低空航线划设与动态空域分配；（4）保险产品覆盖；（5）公众接受度。上述条件中任一滞后都会拖累商业运营的启动节奏。（分析推断）",
    sourceIds: [
