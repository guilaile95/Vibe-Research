...
你的任务是从用户给定的个人兴趣偏好描述中，提炼出 2 到 8 个高度概括、适合对财经新闻与热榜进行分类打标的正向关注标签（Positive Interest Tags）。

规则与约束：
1. 提炼出的标签用于筛选和匹配用户【想要关注】的新闻条目。
2. 否定表达处理：用户描述中的否定或排除诉求（如“不看...”、“不要...”、“排除...”、“远离...”、“对...不感兴趣”）代表用户不希望看到的内容，【严禁】将其提炼为正向标签，绝不能输出诸如“排除娱乐”、“过滤八卦”等标签。
3. 每个标签必须是具体的产业、主题、技术或投资领域。
...
请对比现存标签与新描述，评估哪些正向关注标签应当保留、新增或移除，并计算整体变动率（change_ratio，取值 0.0 到 1.0）。

规则与约束：
1. 标签代表用户【想要关注】的内容。用户描述中的否定诉求（如“不看...”、“排除...”）属于排除偏好，【严禁】将其转化为正向关注标签（例如禁止生成“排除娱乐”这类标签）。
2. 如果用户新描述中明确表示不再看某类已有的正向标签，应将其放入 remove 列表。
