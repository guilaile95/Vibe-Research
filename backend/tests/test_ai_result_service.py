...
def test_canonical_advice_restore_requires_current_matching_account_confirmation(
    tmp_path, monkeypatch
):
    db = tmp_path / "daily_reviews.sqlite3"
    monkeypatch.setattr(service.review_history, "resolve_review_db_path", lambda: db)
    ...
    service.save_portfolio_advice(
        portfolio_data,
        _review(),
        _advice_payload(holdings=[holding], account_funding=funding),
        {"provider": "cli-codex", "model": "gpt"},
    )

    def reality(canonical, confirmation_id="confirmation-a", reasons=None):
        fact = {"confirmation_id": confirmation_id}
        return {
            "canonical": canonical,
            "canonical_reason_codes": reasons or [],
            "cash": {"current_fact": fact},
            "account_total_assets": {"current_fact": fact},
        }

    monkeypatch.setattr(
        service.account_reality_service,
        "get_account_reality",
        lambda: reality(True),
    )
    matched = service.get_ai_result(
        "portfolio_advice", trade_date="2026-07-23", current_portfolio=portfolio_data
    )
    assert matched["stale"] is False
    assert matched["payload"]["holdings"][0]["execution_quantity"] == 100

    cases = [
        (
            lambda: reality(False, reasons=["ACCOUNT_CASH_RECONCILIATION_MISMATCH"]),
            "ACCOUNT_CASH_RECONCILIATION_MISMATCH",
        ),
        (
            lambda: reality(True, confirmation_id="confirmation-b"),
            "ACCOUNT_FUNDING_CONFIRMATION_CHANGED",
        ),
        (
            lambda: (_ for _ in ()).throw(RuntimeError("authority unavailable")),
            "ACCOUNT_REALITY_UNAVAILABLE",
        ),
    ]
    for current_reality, reason in cases:
        monkeypatch.setattr(
            service.account_reality_service,
            "get_account_reality",
            current_reality,
        ...
        assert restored["stale_reason_code"] == reason
        assert restored_holding["execution_quantity"] is None
        assert restored_holding["estimated_amount"] is None
