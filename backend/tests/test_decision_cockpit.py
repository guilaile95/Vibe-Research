...
class TestWatchlistStore:
    def test_initial_not_configured(self, tmp_path, monkeypatch):
        ...
        # 模块级 CACHE_DIR 在 import 时固化：用 monkeypatch 直接改模块属性
...
class TestDecisionCockpitStore:
    ...
    def _db(self, tmp_path, monkeypatch):
        ...
        # review_history.resolve_review_db_path 读 env，但 store 自己读 review_history
        # 直接用模块函数 + 显式 db_path，无需 mock env
    ...
    def test_version_conflict_on_create(self, tmp_path):
        db = self._db_path(tmp_path)
        store.create_plan(db, trade_date="2026-07-24", payload={"v": 1})
        store.create_plan(db, trade_date="2026-07-24", payload={"v": 2})
        with pytest.raises(store.TomorrowPlanConflictError):
            store.create_plan(db, trade_date="2026-07-24", payload={"v": 3}, expected_version=1)
    ...
    def test_one_current_partial_unique_index(self, tmp_path):
        db = self._db_path(tmp_path)
        store.create_plan(db, trade_date="2026-07-24", payload={"v": 1})
        store.create_plan(db, trade_date="2026-07-24", payload={"v": 2})
        # 直接查 DB 确认至多一个 current
        import sqlite3
...
class TestDecisionCockpitSignals:
    def test_value_pe_pb_percentile(self):
        ...
        fin = {"revenue_yoy": 12, "net_profit_yoy": -15, "roe": 16, "gross_margin": 40, "op_cf_ps": 1.2}
        sigs = signals.evaluate_value("600519", val, fin)
        by_label = {s["label"]: s for s in sigs}
        assert by_label["pe_ttm_percentile"]["assessment"] == "strong"  # pct<=20
        assert by_label["pb_percentile"]["assessment"] == "weak"  # pct>=80
    ...
    def test_trend_uptrend_strong(self):
        ...
        for i in range(80):
            p *= 1.005
            bars.append({"open": p * 0.99, "close": p, "high": p * 1.01, "low": p * 0.98, "volume": 1000000})
    ...
    def test_candidate_pool_protected_sources(self):
        pool = signals.build_candidate_pool(
            ...
            [{"code": "000003"}],
            [{"code": "000004"}],
            max_candidates=4,  # 受保护源 3 个保留，只剩 1 个名额给填充
        )
        codes = [c["code"] for c in pool]
        # 受保护源（holding/watchlist/sector）必须全部保留
    ...
    def test_cash_exec_lot_floor(self):
        # 1000 shares @ 100, cash 50000 → 5 lots (500 shares), cost 50000
        r = signals.compute_cash_exec(1000, 100.0, 50000)
        assert r["lots"] == 5 and r["shares"] == 500
        # 超出可用现金
        r2 = signals.compute_cash_exec(1000, 100.0, 30000)
        assert r2["lots"] == 3 and r2["shares"] == 300
        # 非法输入
        r3 = signals.compute_cash_exec(None, 100.0, 50000)
        assert r3["executable"] is False
