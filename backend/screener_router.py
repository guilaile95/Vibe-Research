...
def full_market_endpoint(
    ...
    try:
        parsed_filters = json.loads(filters) if filters is not None else None
        if filters is not None and not isinstance(parsed_filters, list):
            raise rdp.ResearchDataPlaneQueryValidationError("filters must be a JSON array")
        ...
        raise HTTPException(status_code=422, detail="filters must be a JSON array") from exc
