...
- 用户明确授权一项任务后，该授权默认覆盖：使用任务分支或 worktree、修改任务范围内代码与文档、运行测试、创建普通提交、推送任务分支、创建或更新 Draft PR，以及修复本轮审查发现的问题。通过验收后，授权同时覆盖转 Ready、普通合并 PR 到已确认的目标稳定分支、检查合并后 exact stable CI，以及更新该任务已授权的本机运行入口；这些交付步骤无需再次授权。
- 仍需单独明确授权的操作：绕过 PR 直接修改或推送稳定分支、force push，以及删除 worktree、分支、备份或其他破坏性清理。已有授权不扩大到这些操作；不可逆操作仍遵守用户确认口令要求。
...
- 只创建普通新提交。
- 已授权任务通过验收后，可转 Ready 并普通合并 PR 到已确认的目标稳定分支，无需再次授权；保留提交历史，不使用 squash / rebase merge。
- 默认不绕过 PR 直接修改稳定分支；不直接推送 main / 稳定分支。
