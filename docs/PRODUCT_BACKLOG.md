...
|----|-----|
| **授权状态** | 候选（未授权）|
| **当前实现** | **已有实质实现**，是仓库内最完整的候选之一 |
| **已实现组件** | 行业/概念/区域排名（`board_ranking`）；个股 + 行业资金流（`stock_fund_flow_120d`、`_sectors`）；龙虎榜席位/机构净买；融资融券；大宗交易；股东户数变化；分红/解禁；行业动态数据面板 + 报告发现 |
...
**MVP 范围（若授权）**：
1. 北向资金数据接入（`a-stock-data` 技能北向净流入/个股北向持仓变化）
2. 北向资金时序图 + 与股价叠加
...
| 5 | **BK-05 论文漂移** | 已有实质实现；漂移检测为高价值增量 |
| 6 | **BK-09 数据健康增强** | 需与 BK-01 联动 |
| 7 | **BK-06 巴菲特方法论** | 概念清晰但实现链长；依赖外部技能对接 |
| 8 | **BK-07 Provider 抽象** | 架构改进，用户透明 |
| 9 | **BK-08 视觉 a11y** | 视觉体系已成熟；a11y 为合规需求 |
| 10 | **BK-10 工程治理** | 工程类非产品；长期可维护性 |
