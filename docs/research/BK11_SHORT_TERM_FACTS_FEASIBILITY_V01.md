...
以 2026-07-28 / 2026-07-29 / 2026-07-30 三个连续交易日为审计样本（基于项目已有调用路径和 a-stock-data SKILL 实测记录）：

### 5.1 验证矩阵

| 指标 | 07-28 | 07-29 | 07-30 | 跨源一致性 |
|------|-------|-------|-------|-----------|
| zt_count（涨停池） | ✓ 非空 | ✓ 非空 | ✓ 非空 | 东财池与同花顺涨停揭秘数量偏差 < 5%（同花顺含 ST） |
| zb_count（炸板池） | ✓ 非空 | ✓ 非空 | ✓ 非空 | 单源（东财），无交叉验证 |
| dt_count（跌停池） | ✓ 非空 | ✓ 非空 | ✓ 非空 | 单源（东财） |
...
```text
涨停/炸板/跌停/昨涨停：单源（东财 push2ex），无独立第二源交叉验证。
...
```text
Slice 1+ 的单元测试使用 fixture 中的 normal/partial/unavailable 场景。
不依赖网络即可验证指标计算、降级逻辑、Preflight 规则。
fixture 中的数字为审计时真实数据的脱敏快照，非随机生成。
```
