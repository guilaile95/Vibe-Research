...
### 0.1 最终审查修复（2026-07-23）

- `POST /api/daily-review/analyze` 的业务流包装改为 async generator，并由独立 ASGI disconnect 监听器维护线程安全取消信号；同步上游按单事件在线程池中迭代，SQLite UPSERT 在事务开始前、写入后和提交前检查同一信号，断开即回滚。真实 ASGI 回归同时覆盖上游完成前断开和保存已经开始后断开，均不保存、不发送业务层 `done`。
- 持仓建议恢复与生成使用分离的请求令牌。定时刷新和持仓增删改在生成运行中只刷新持仓数据，不把建议状态从 `running` 改回 `restoring`，不重新启用生成按钮，也不使生成响应失效；生成完成后必须按本次 `trade_date` 权威重读已保存结果，旧恢复/旧生成响应均不能覆盖新状态。
- `run_cli_stream()` 提前关闭时执行 `kill + wait`，关闭父进程持有的 stdin/stdout，并等待 stdout 读线程退出；disconnect 信号在无后续 stdout 时以 50ms 间隔协作取消。测试同时覆盖调用顺序、真实子进程回收和“输出一行后卡住”的 ASGI 断开时序。
- `ai_result_store.get_result()` / `get_latest_result()` 使用 SQLite `mode=ro`。数据库文件或 `ai_generated_results` 表不存在时返回 `None`，不创建目录、数据库或表。
- 恢复结果在服务层按 `result_type`、持久层 schema、业务 payload schema、日期/时间和持仓指纹重新校验。持仓建议的保存与恢复共用版本化严格契约，逐层校验 summary、账户动作、逐股建议、枚举、有限数字、字符串数组和账户资金，拒绝未知字段；语义损坏统一抛出不含字段值、路径或 SQL 的安全异常，API 继续返回固定安全错误。
- 最终审查专项后端回归为 164 项通过；完整离线后端回归为 957 项通过、11 项 live 测试跳过；新增前端竞态纯逻辑测试 3 项通过，生产构建通过。
